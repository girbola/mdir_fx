...
//			pstmt.closeOnCompletion();
     ...
     */
    // @formatter:on
    public static boolean deleteFileInfoListToDatabase(Connection connection, List<FileInfo> list) throws SQLException {
        Messages.sprintf("deleteFileInfoListToDatabase tableCreated Started");
        if (connection.isClosed()) {
            Messages.sprintfError("Connectino were closed");
            return false;
        }
        try {
            if (!connection.getAutoCommit()) {
