...
public class FileInfoSqlConnectionFactory {
    ...
    private static void showConnections(List<Connection> connectionList) {
        Messages.sprintf("FileInfo Database ConnectionList size is: " + connectionList.size());
        Iterator<Connection> iterator = connectionList.iterator();
        while (iterator.hasNext()) {
            Connection conn = iterator.next();
            try {
                if (conn != null && !conn.isClosed()) {
                    Messages.sprintf("FileInfo SQLite database connection is: " + conn.getMetaData().getURL());
                } else {
                    iterator.remove(); // Remove closed connections from the list
                }
            } catch (SQLException e) {
                iterator.remove(); // Remove problematic connections
                Messages.sprintf("FileInfo Error accessing connection: " + e.getMessage());
            }
        }
    }
