    ...
    private void chooseFolder_workDir_action(ActionEvent event) {
        ...
        if (file != null) {
            if (Files.exists(file.toPath())) {
                ...
                boolean hasFilePermissions = FileUtils.checkFilePermissions(file);
                if (!hasFilePermissions) {
                    Messages.warningText(Main.bundle.getString("noFilePermissions"));
                    return;
                }
                WorkDirSQL workDirSQL = new WorkDirSQL(Paths.get(file.getAbsolutePath()));

                Messages.sprintf("folder path is now: " + file.toString());
                Main.conf.setWorkDir(file.toString());

                if (workDirSQL.getConnection() == null) {
                    Messages.sprintfError("Error creating workdir connection");
                    return;
                }
