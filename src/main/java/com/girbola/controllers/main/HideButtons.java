...
public class HideButtons {
    ...
    private boolean sorted_show = false;
	...
	button.getScene().getAccelerators().put(new KeyCodeCombination(numberKey, KeyCombination.CONTROL_DOWN),
		(Runnable) () -> {
			...
			button.fire();
			hide_show_table(button, TableType.ASITIS);

		    } else if (tableType.equals(TableType.SORTED)) {
			button.fire();
			hide_show_table(button, TableType.SORTED);
		    } else if (tableType.equals(TableType.SORTIT)) {
			button.fire();
			hide_show_table(button, TableType.SORTIT);
		    }
		});
    }

    void hide_show_table(Button button, TableType tableType) {
