...
public class TableRowSelector implements Callback<TableView<EXIF_Data_Selector>, TableRow<EXIF_Data_Selector>> {
    ...
    public TableRow<EXIF_Data_Selector> call(TableView<EXIF_Data_Selector> arg0) {
        TableRow<EXIF_Data_Selector> row = new TableRow<>();
        row.setOnMouseClicked((MouseEvent e) -> {
            String dateValueYMD = table.getSelectionModel().getSelectedItem().getInfo();
            if (e.getButton() == MouseButton.PRIMARY && e.getClickCount() == 2) {
                ...
                for (Node node : tilePane.getChildren()) {
                    if (node instanceof VBox && node.getId().equals("imageFrame")) {
                        ...
                        LocalDate localDateTaken = DateUtils.longToLocalDateTime(fileInfo.getDate()).toLocalDate();
                        String formattedDate = localDateTaken.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                        if (formattedDate.equals(dateValueYMD)) {
                            GUI_Methods.scrollNodeTopIntoView(scrollPane, node, 0);
                            // Break if you only want to scroll to the first matching VBox
                            break;
                        }
                    }
//					if (table.getId().equals("dates_tableView")) {
//						value = GUI_Methods.getDate(node);
//						Messages.sprintf("dates_tableView value: " + value);
//						if (table.getSelectionModel().getSelectedItem().getInfo().contains(value)) {
//							VBox vbox = (VBox) node;
//							double vvalue = GridPane.getRowIndex(vbox) * vbox.getHeight()
//									+ (tilePane.getVgap() * (GridPane.getRowIndex(vbox) - 2));
