...
//	private DriveInfo driveInfo;
	...
	public ScanDrives(CheckBoxTreeItem aRootItem, ObservableList<Path> aDrivesList_selected_obs,
			...
			return new Task<Integer>() {
				...
				private void redrawRootFolders() throws IOException {
					for (DriveInfo driveInfo : rootDrives) {
						...
						Messages.sprintf("Iterating root drives: " + driveInfo.getDrivePath() + " drive serial: " + driveInfo.getIdentifier());
...
//						täytyy listofroot olemassa oleva mihin verrata uutta listaa(eli tätä listaa)
					}
					
					for (DriveInfo driveInfo : setOfRootDrives) {
						if (!findDuplicateDrive(driveInfo)) {
							Messages.sprintf("Adding all to root Drives. DriveInfo: " + driveInfo.getDrivePath() + " serial: " + driveInfo.getIdentifier() + " setOfRootDrives size: " + setOfRootDrives.size());
